...
public abstract class AbstractScreen extends JPanel implements Screen {
    ...
    public static void dispose() {
        if(sFrame==null)
            throw new IllegalStateException("No screen has been visible yet.");
        sFrame.dispose();
    }
    ...
    protected final <C extends JComponent> ComponentBuilder<C> addComponentBuilder(C comp, Object param) {
     ...
     */
    @Deprecated
    protected final <C extends JComponent> ComponentBuilder<C> addComponentBuilder(C comp, Object param, JComponent... children) {
    ...
    public AbstractScreen visible(boolean fullscreen) {
        if (sFrame == null) {
            sFrame = fullscreen ?
                    UI.createFullscreenFrame(this, title(), icon()) :
                    UI.createFrame(this, title(), icon());
        }
    ...
    private final class ComponentAdderBuilder<C extends JComponent> extends AbstractComponentBuilder<C> {
        ...
        public C build() {
            ...
            if(mParam == null) add(component);
            else add(component, mParam);
            return component;
        }
    }
}
